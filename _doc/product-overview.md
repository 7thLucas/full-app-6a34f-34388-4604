...
## Product
A lightweight, browser-based two-player Tic-Tac-Toe game. Players take turns marking X and O on a 2×2 grid. The first to align two marks (row, column, or diagonal) wins. If all four cells fill with no winner, the game ends in a draw. Score tracking persists across rounds in the same session.
